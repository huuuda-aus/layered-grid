...
var LayeredGrid = react.forwardRef(
  function LayeredGrid2(props, ref) {
    ...
    const onCellSelectIntentRef = react.useRef(onCellSelectIntent);
    ...
    const [animatedView, setAnimatedView] = react.useState(() => ({
      focusDepth: Number.isFinite(state.camera.focusDepth) ? state.camera.focusDepth : 0,
      modeBlend: state.mode === "depth" ? 1 : 0
    }));
    const [hoveredCell, setHoveredCell] = react.useState(null);
    const lastPointerClientPosRef = react.useRef(null);
    const [isPointerInteractionActive, setIsPointerInteractionActive] = react.useState(false);
    const [isMomentumActive, setIsMomentumActive] = react.useState(false);
    const [cameraTweenFrame, setCameraTweenFrame] = react.useState(null);
    const panSessionRef = react.useRef(null);
    const lastPointerDownInfoRef = react.useRef(null);
    const processedExternalEventKeysRef = react.useRef(/* @__PURE__ */ new Set());
    const transitionFrameRef = react.useRef(null);
    const animatedViewRef = react.useRef(animatedView);
    const transitionFromRef = react.useRef(animatedView);
    const transitionStartTimeRef = react.useRef(null);
    const interactionCameraRef = react.useRef(state.camera);
    const committedCameraRef = react.useRef(state.camera);
    const pointerCameraFrameRef = react.useRef(null);
    const momentumFrameRef = react.useRef(null);
    const recenterCameraFrameRef = react.useRef(null);
    const recenterCameraStartTimeRef = react.useRef(null);
    const recenterCameraFromRef = react.useRef(state.camera);
    const recenterCameraToRef = react.useRef(state.camera);
    const recenterPendingCommitRef = react.useRef(null);
    const suppressSelectionRecenterKeyRef = react.useRef(null);
    ...
    const queuedPointerCameraRef = react.useRef(null);
    ...
    const mergedVisual = react.useMemo(() => resolveLayeredGridVisualConfig(visual), [visual]);
    const mergedEffects = react.useMemo(() => resolveLayeredGridEffectsConfig(effects), [effects]);
    ...
    const displayCamera = react.useMemo(() => ({
      ...effectiveCamera,
      scale: clampedScale
    ...
    const stopRecenteringAnimation = react.useCallback(() => {
    ...
    const handleExternalEventRef = react.useRef(handleExternalEvent);
    ...
    const animateCameraIntent = react.useCallback((targetCamera, reason) => {
    ...
    const animateCameraIntentRef = react.useRef(animateCameraIntent);
    animateCameraIntentRef.current = animateCameraIntent;
    const animateSelectionZoomFromPointer = react.useCallback((args) => {
      const viewport = viewportRef.current;
      if (!viewport) {
        const centered = centerCameraOnCell({
          row: args.row,
    ...
    return /* @__PURE__ */ jsxRuntime.jsxs("section", { className, style: { position: "relative", ...style }, children: [
      /* @__PURE__ */ jsxRuntime.jsxs("div", { style: hudStyle, children: [
